                  ...
                  <a href="#add" class="btn btn-primary mb-4" id="add_data"><i class="fa fa-plus"></i> Add Application</a>
                    <div class="table-responsive">
                      ...
                      <input type="hidden" class="form-control security_token" name="{{ $token_name }}" value="{{ $token }}">
                        ...
                        <tbody>
                            @foreach($get_apps as $data)
                            <tr>
                                <td>{{ $data->application_id }}</td>
                                <td>{{ $data->application_name }}</td>
                                <td><img src="{{ base_url("assets/img/portfolio/"). $data->walpaper }}" style="height:200px;width:200px;" class="img-fluid"></td>
                                <td>{{ $data->project_date }}</td>
                                <td>{{ $data->type }}</td>
                                <td>{{ $data->date_time }}</td>
                                <td>
                                    <a onclick="popUpUpdate({{ $data->application_id }}, 'apps');" class="btn btn-primary text-white"><i class="fa fa-edit"></i></a>
                                    <a onclick="deleteData({{ $data->application_id }})" class="btn btn-danger text-white"><i class="fa fa-trash"></i></a>
                                    <a href=" {{ base_url('dashboard/images/'. $data->application_id). '/'. $data->application_name }} " class="btn btn-warning text-white"><i class="fa fa-image"></i></a>
                                </td>
                            </tr>
                            @endforeach 
                        </tbody>
